...
def get_triples_from_llm(image_paths, prompt):
    ...
    bev_image_blocks = []
    try:
        lidar_image_path = [p for p in image_paths if p.lower().endswith('.ply')]
        if lidar_image_path:
            bev_image_path = BEV_generator.convert_ply_to_png(lidar_image_path[0])
            bev_image_blocks = [{"type": "image_url", "image_url": {
...
for scenario in SCENARIOS:
    ...
    for weather in os.listdir(scenario_folder):
        ...
        if not os.path.isdir(rgbs_folder):
            continue
        lidar_images_folder = os.path.join(weather_folder, "second_car_lidar")
        ...
        lidar_images = sorted(glob.glob(os.path.join(lidar_images_folder, "*.ply")))
        ...
        while (loop * BATCH_SIZE) < len(rgb_images):
            ...
            triples, call_metrics = get_triples_from_llm(
                selected_images, prompt if not lidar_images_selected else prompt2
            )
