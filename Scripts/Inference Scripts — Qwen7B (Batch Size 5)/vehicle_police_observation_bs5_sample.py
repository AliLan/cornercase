...
def get_triples_from_llm(image_paths, prompt):
    ...
    call_metrics = {
        ...
        "success": success, "is_timeout": is_timeout, "error": error,
    }
...
BATCH_SIZE       = 5
MAX_SAMPLE_LOOPS = 10   # API calls per scenario/weather combination
PROGRESS_EVERY   = 5
VEHICLE          = "police"
...
# 120s timeout — fast-fail on Ollama crashes
...
def sample_loops_done(vehicle, scenario, weather, batch_size, attempt):
    """Count how many API calls are already recorded for this sample attempt."""
    count = 0
    for d in [CORNERCASE_DIR, METRICS_DIR]:
        for fpath in glob.glob(os.path.join(d, "_metrics_*.jsonl")):
            try:
...
Always include:
ex:vehiclePolice_activity a prov:Activity .
...
ex:occ1 a avcco:OcclusionEvent ;
    ...
    prov:wasGeneratedBy ex:vehiclePolice_activity ;
...
for scenario in SCENARIOS:
    ...
    for weather in sorted(os.listdir(scenario_folder)):
        ...
        sample_count = sample_loops_done(VEHICLE, scenario, weather, BATCH_SIZE, ATTEMPT)

        weather_folder = os.path.join(scenario_folder, weather)
        if not os.path.isdir(weather_folder):
            continue

        if sample_count >= MAX_SAMPLE_LOOPS:
            print(f"[SKIP] {scenario}/{weather} already has {sample_count} sample loops recorded.")
            continue

        print(f"Processing scenario: {scenario}, weather: {weather}, vehicle: {VEHICLE}")

        rgbs_folder = os.path.join(weather_folder, "police_rgb")
        if not os.path.isdir(rgbs_folder):
            print(f"  Skipping: police_rgb not found in {weather_folder}")
            continue
        ...
        print(f"\n[START] vehicle={VEHICLE}  {scenario}/{weather}  images={len(rgb_images)}"
              f"  resume_loop={loop}  sample_count={sample_count}/{MAX_SAMPLE_LOOPS}")

        while loop < len(rgb_images) and sample_count < MAX_SAMPLE_LOOPS:
            print(f"Loop: {loop + 1}/{len(rgb_images)}  sample={sample_count + 1}/{MAX_SAMPLE_LOOPS}")

            selected_images = rgb_images[loop:loop+BATCH_SIZE]
            ...
            if call_metrics["is_timeout"]:
                status = "timeout_error"
            elif not call_metrics["success"]:
                status = "api_error"
            ...
            with open(metrics_file, "a") as mf:
                mf.write(json.dumps({
                    ...
                    "vehicle":            VEHICLE,
                    "status":             status,
                    "is_timeout":         call_metrics["is_timeout"],
            ...
            sample_count += 1
            _done = loop + 1
            _is_sample_done = sample_count >= MAX_SAMPLE_LOOPS
            _pct = min(100.0, round(sample_count / MAX_SAMPLE_LOOPS * 100, 1))
            _tag = "[SAMPLE_DONE]" if _is_sample_done else "[PROGRESS]   "
            elapsed_total = round(time.time() - _t_run_start, 1)
            print(f"{_tag} vehicle={VEHICLE}  {scenario}/{weather}  "
                  f"sample={sample_count}/{MAX_SAMPLE_LOOPS} ({_pct}%)  status={status}  "
                  f"latency={call_metrics['latency_s']:.1f}s  total={elapsed_total}s")
            ...
            loop_output_path = os.path.join(CORNERCASE_DIR, "output", scenario, weather, str(loop + 1))
...
print(f"\n[COMPLETE] vehicle={VEHICLE}  batch_size={BATCH_SIZE}  total_elapsed={total_elapsed}s  ({round(total_elapsed/3600,2)}h)")
