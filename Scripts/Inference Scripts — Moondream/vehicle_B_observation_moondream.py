...
SCENARIOS = ["bus_obscuring_car_second_car", "bus_stop_near_playground"]
...
Always include:
ex:vehicleB_activity a prov:Activity .
...
ex:occ1 a avcco:OcclusionEvent ;
    ...
    prov:wasGeneratedBy ex:vehicleB_activity ;
...
for scenario in SCENARIOS:
    ...
    for weather in sorted(os.listdir(scenario_folder)):
        main_graph = Graph()
        loop = completed_loops("B", scenario, weather, BATCH_SIZE, ATTEMPT)
        ...
        rgbs_folder = os.path.join(weather_folder, "bus_rgb")
        ...
        print(f"\n[START] vehicle=B  {scenario}/{weather}  images={len(rgb_images)}  resume_loop={loop}")
        ...
        while loop < len(rgb_images):
            ...
            if _done % PROGRESS_EVERY == 0 or _is_last:
                ...
                print(f"{_tag} vehicle=B  {scenario}/{weather}  "
            ...
            loop_output_file = os.path.join(loop_output_path, "vehicle_B_observations_loop.ttl")
            try:
            ...
            main_output_file = os.path.join(loop_output_path, "vehicle_B_observations.ttl")
            try:
