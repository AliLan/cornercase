...
REVISION   = "2025-01-09"
BATCH_SIZE = 1   # Moondream processes one image at a time
PROGRESS_EVERY = 10

CORNERCASE_DIR = os.path.dirname(os.path.abspath(__file__))
METRICS_DIR    = os.path.join(CORNERCASE_DIR, "metrics_new", "moondream")
os.makedirs(METRICS_DIR, exist_ok=True)
metrics_file = os.path.join(METRICS_DIR, f"_metrics_{random.randint(10**18, 10**19)}.jsonl")

print(f"Loading {MODEL_ID} (first run downloads ~2GB)...")
_t_load = time.time()
...
def get_triples_from_llm(image_paths, prompt):
    ...
    image_path = rgb_image_paths[0]  # Moondream: one image per call
    ...
    try:
        ...
        raw_output = md_model.answer_question(enc, prompt, md_tokenizer)
    except Exception as e:
        success = False
...
SCENARIOS = ["bus_obscuring_car_second_car"]  # bus_stop_near_playground has no second_car_rgb
...
Always include:
ex:vehicleC_activity a prov:Activity .
...
ex:occ1 a avcco:OcclusionEvent ;
    ...
    prov:wasGeneratedBy ex:vehicleC_activity ;
...
for scenario in SCENARIOS:
    ...
    for weather in sorted(os.listdir(scenario_folder)):
        main_graph = Graph()
        loop = completed_loops("C", scenario, weather, BATCH_SIZE, ATTEMPT)
        ...
        rgbs_folder = os.path.join(weather_folder, "second_car_rgb")
        ...
        print(f"\n[START] vehicle=C  {scenario}/{weather}  images={len(rgb_images)}  resume_loop={loop}")

        while loop < len(rgb_images):
            print(f"Loop: {loop + 1}, RGB images: {len(rgb_images)}")
            ...
            if _done % PROGRESS_EVERY == 0 or _is_last:
                ...
                print(f"{_tag} vehicle=C  {scenario}/{weather}  "
            ...
            loop_output_path = os.path.join(CORNERCASE_DIR, "outputs_new", "moondream", scenario, weather, str(loop + 1))
            os.makedirs(loop_output_path, exist_ok=True)

            loop_output_file = os.path.join(loop_output_path, "vehicle_C_observations_loop.ttl")
            try:
            ...
            main_output_file = os.path.join(loop_output_path, "vehicle_C_observations.ttl")
            try:
