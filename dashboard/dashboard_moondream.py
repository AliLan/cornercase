"""
Moondream Benchmark Monitor
Live dashboard for vehicle_A/B/C_observation_moondream.py runs.
Run: python dashboard_moondream.py  →  open http://localhost:8051
Reads ONLY from:
...
OUTPUT_DIR     = os.path.join(CORNERCASE_DIR, "outputs_new", "moondream")
VEHICLES       = ["A", "B", "C"]
REFRESH_MS     = 5000
...
app = Dash(__name__, title="Moondream Monitor")
app.layout = html.Div(
    ...
    children=[
        ...
        html.Div(
            ...
            children=[
                html.Div([
                    html.H1("Moondream Benchmark Monitor",
                            style={"margin": 0, "fontSize": "22px", "fontWeight": 700}),
                    html.P("vikhyatk/moondream2  ·  HuggingFace MPS local inference",
                           style={"margin": "4px 0 0", "color": COLORS["model"],
                                  "fontSize": "13px"}),
                ]),
                html.Div(id="md-last-refresh",
        ...
        # Global progress bar
        html.Div(id="md-global-bar", style={"marginBottom": "24px"}),

        # Vehicle cards
        html.Div(id="md-vehicle-cards",
                 style={"display": "grid",
                        "gridTemplateColumns": "repeat(3, 1fr)",
                        "gap": "16px", "marginBottom": "24px"}),

        # File paths panel
        html.Div(id="md-file-paths",
        ...
        html.Div([
            ...
            html.Div(id="md-activity-table"),
        ], style={"backgroundColor": COLORS["card"], "borderRadius": "12px",
                  "padding": "20px", "border": f"1px solid {COLORS['border']}"}),

        dcc.Interval(id="md-interval", interval=REFRESH_MS, n_intervals=0),
    ]
)
...
def is_running(vehicle):
    target = f"vehicle_{vehicle}_observation_moondream.py"
...
@app.callback(
    Output("md-global-bar",    "children"),
    Output("md-vehicle-cards", "children"),
    Output("md-file-paths",    "children"),
    Output("md-activity-table","children"),
    Output("md-last-refresh",  "children"),
...
if __name__ == "__main__":
    print("Moondream Monitor running at http://localhost:8051")
