"""
GLM-OCR Benchmark Monitor
Live dashboard for vehicle_A/B/C_observation_glm.py runs.
Run: python dashboard_glm.py  →  open http://localhost:8052
Reads ONLY from:
  metrics_new/glm/_metrics_*.jsonl
  outputs_new/glm/
"""
...
METRICS_DIR    = os.path.join(CORNERCASE_DIR, "metrics_new", "glm")
OUTPUT_DIR     = os.path.join(CORNERCASE_DIR, "outputs_new", "glm")
VEHICLES       = ["A", "B", "C"]
REFRESH_MS     = 5000
...
app = Dash(__name__, title="GLM-OCR Monitor")
app.layout = html.Div(
    ...
    children=[
        ...
        html.Div(
            ...
            children=[
                html.Div([
                    html.H1("GLM-OCR Benchmark Monitor",
                            style={"margin": 0, "fontSize": "22px", "fontWeight": 700}),
                    html.P("zai-org/GLM-OCR  ·  HuggingFace local inference",
                           style={"margin": "4px 0 0", "color": COLORS["model"],
                                  "fontSize": "13px"}),
                ]),
                html.Div(id="glm-last-refresh",
        ...
        # Global progress bar
        html.Div(id="glm-global-bar", style={"marginBottom": "24px"}),

        # Vehicle cards
        html.Div(id="glm-vehicle-cards",
                 style={"display": "grid",
                        "gridTemplateColumns": "repeat(3, 1fr)",
                        "gap": "16px", "marginBottom": "24px"}),

        # File paths panel
        html.Div(id="glm-file-paths",
        ...
        html.Div([
            ...
            html.Div(id="glm-activity-table"),
        ], style={"backgroundColor": COLORS["card"], "borderRadius": "12px",
                  "padding": "20px", "border": f"1px solid {COLORS['border']}"}),

        dcc.Interval(id="glm-interval", interval=REFRESH_MS, n_intervals=0),
    ]
)
...
def is_running(vehicle):
    target = f"vehicle_{vehicle}_observation_glm.py"
...
@app.callback(
    Output("glm-global-bar",    "children"),
    Output("glm-vehicle-cards", "children"),
    Output("glm-file-paths",    "children"),
    Output("glm-activity-table","children"),
    Output("glm-last-refresh",  "children"),
    Input("glm-interval",       "n_intervals"),
)
def refresh(_):
...
if __name__ == "__main__":
    print("GLM-OCR Monitor running at http://localhost:8052")
